...
import { ArrowLeft, Loader2, User, FileText, Film, Music } from 'lucide-react';
...
function ReadOnlyNode({ doc, canvasId }) {
  ...
  return (
    <div
      className="absolute rounded-xl border border-surface-500 bg-surface-800 shadow-lg shadow-black/30 flex flex-col items-center justify-center overflow-hidden select-none"
      style={{
        left: doc.position_x,
        top: doc.position_y,
        width: NODE_W,
    ...
    >
      <div className="w-full flex-1 flex items-center justify-center overflow-hidden rounded-t-xl bg-surface-700">
        {thumbUrl ? (
          <img
            src={thumbUrl}
    ...
    <div className="min-h-screen bg-surface-900 text-white flex flex-col">
      ...
      <div className="flex-1 overflow-auto bg-surface-900">
        <div className="relative" style={{ width: canvasWidth, height: canvasHeight, minWidth: '100%', minHeight: 'calc(100vh - 56px)' }}>
          ...
          {docs.map(doc => (
            <ReadOnlyNode key={doc.id} doc={doc} canvasId={id} />
          ))}
        </div>
      </div>
